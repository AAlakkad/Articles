...
صفحات الويب التي نراها في المتصفح تتألف من مكونات متعددة؛ كأطر العمل، مكتبات، ملفات JavaScript وCSS وغيرها.
...
دعنا نرى كيف يمكن ﻷداة Bower أن تساعد في اختصار الوقت. بعد تنصيب الأداة على الحاسب، يمكن تنزيل جميع ما سبق باستخدام الأمر التالي:
```bash
bower install jquery bootstrap fontawesome
```
فقط! بهذا نكون قمنا بتنصيب المتطلبات الثلاثة معاً، ويمكن البدء باستخدامها دون هدر وقت بتكرار الخطوات التقليدية. ومهما ازداد عدد المتطلبات يبقى تنصيبها بنفس هذه الطريقة البسيطة.
...
في الواقع نجد أن معظم إضافات وتحسينات أطر العمل والمكتبات الشهيرة تستخدم هذه الأداة، وذلك لضمان استخدام نسخة محددة من إطار العمل أو المكتبة التي تم إنشاء هذه الإضافة لها. لتجنب مشاكل في عدم التوافقية بينهما.
...
### تنصيب NodeJS
بالنسبة لمستخدمي ويندوز وماك، يمكن التوجه لموقع [nodejs](http://nodejs.org/download/) وتحميل ملف التنصيب المناسب حسب نظام التشغيل.

إن كنت تعمل على لينكس يمكنك اتباع الطريقة السابقة. أو يمكن تنصيب nodejs عن طريق المستودعات الخاصة بنظام التشغيل، مثلاً إن كنت تستخدم أبونتو (Ubuntu) أو أحد مشتقاته، يمكن تنصيب nodejs عن طريق سطر الأوامر:
```bash
...
### سطر الأوامر
إن كنت تعمل على ويندوز فسطر الأوامر هو واجهة dos أو اختصاراً هو `cmd`. وإن كنت تعمل على لينكس فهناك عدد من البرامج لمحاكاة سطر الأوامر أشهرها `Terminal` أو `Gnome Terminal` بحسب التوزيعة والواجهة التي تستخدمها. أما إن كنت تعمل على ماك فيمكن استخدام `iTerm`.
...
npm install -g bower
```
**ملاحظة:** إن كنت تعمل على نظام لينكس أو ماك وفشلت عملية التنصيب قد تحتاج لإضافة أمر `sudo` في البداية ليتم استخدام صلاحيات المسؤول (root) في التنصيب، فيصبح الأمر:
```bash
...
في الطريقة الأولى عندما قمنا بتنصيب jQuery لم نقم بتحديد الإصدار الذي نريده. بهذه الحالة يقوم Bower تلقائياً بتحميل آخر نسخة من المكوّن المطلوب. ويمكن أيضاً أن نفعل كما في المثال الثاني حيث قمنا بتحديد النسخة المطلوبة بذاتها.
...
بشكل افتراضي لا يكون أي خيار محدداً. يمكن تحديد أي خيار منها عن طريق تحريك المؤشر باستخدام السهمين أعلى وأسفل، ثم ضغط زر المسافة لتحديد الخيار/الخيارات المطلوبة. ثم نضغط زر `Enter` للمتابعة إلى السؤال التالي.

المقصود من هذا السؤال هو تحديد آلية عمل الوحدات المكونة للمشروع (modules) عند كونه مشروع JavaScript متقدّم.
...
بشكل افتراضي يأخذ Bower الاسم والبريد الإلكتروني من إعدادات git الموجودة على الحاسب.

```
? license: MIT
```
رخصة استخدام المشروع بشكل مختصر. إن كنت مهتماً بمعرفة أشهر الرخص ومتى تَستخدم كل واحدة يمكن الذهاب لموقع [Choose an OSS license](http://choosealicense.com) يحوي توضيح بالإنكليزية.
```
? homepage: 
```
...
يمكن استخدام Bower في أي مشروع ويب يحتاج إلى واجهات تظهر للمستخدم (Front-end). سواء كانت المكونات للاستخدام المباشر أو لتتم معالجتها واستخدامها في بناء مكوّنات مخصصة بناءً على متطلبات التصميم.
