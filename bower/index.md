...
تتألّف صفحات الويب التي نراها على المتصفح من مكوِّنات متعددة؛ كأطر العمل، مكتبات، ملفات JavaScript وCSS وغيرها.

[Bower](http://bower.io) أداة لإدارة هذه المكوِّنات -الحزم- المُخصّصة للويب، كالتنصيب والتحديث والحذف وغيرها من عمليات الإدارة.

Bower يشبه إلى حد كبير تلك الأدوات الأخرى المخصّصة لإدارة الحزم، مثل gem للغة Ruby وcomposer لـ PHP و npm لـ nodejs.

## لكن ما الفائدة منه؟ ألا يمكنني متابعة عملي كما اعتدت من دونه؟

يمكننا الاستفادة من أداة Bower بعددٍ من الفوائد، لنفرض أن مشروعك القادم يحتاج إلى استخدام مكتبة [jQuery](http://jquery.com) وإطار عمل [Twitter Bootstrap](http://getbootstrap.com) وخطّ الأيقونات [FontAwesome](http://fortawesome.github.io/Font-Awesome).

لتحميل كل واحدة من المُتطلّبات يقوم المطور بما يلي:
- الذهاب إلى موقع المكتبة أو إطار العمل
- التوجه إلى زرّ التحميل
- تحميل الملف المضغوط
- فك ضغط الملف المضغوط في مجلد المشروع

لو كان المشروع يحتاج إلى 3 أدوات فقط سيقوم المطور بتكرار الخطوات السابقة 3 مرات، ولو كان يحتاج 10 أدوات سيقوم بتكرار الخطوات 10 مرات! هل ترى حجم العمل المطلوب فقط لتحميل المُتطلّبات؟
...
```
هذا كل ما في الأمر.
بهذا نكون قمنا بتنصيب المُتطلّبات الثلاثة معاً، ويمكن البدء باستخدامها دون هدر وقت بتكرار الخطوات التقليدية. ومهما ازداد عدد المُتطلّبات يبقى تنصيبها بنفس هذه الطريقة البسيطة.

من ميزات هذه الأداة تحديد الإصدار المطلوب للمكوِّنات (سواء كانت مكتبات أو أطر العمل). إن كنتَ تريد مثلاً الإصدار رقم 3.1.1 من إطار عمل Twitter Bootstrap فيمكن أن تحدد هذا في أمر التنصيب نفسه، أما بالطريقة التقليدية فتحتاج إلى البحث هنا وهناك لتجد مبتغاك!
...
نجد أن معظم إضافات وتحسينات أطر العمل والمكتبات الشهيرة تستخدم هذه الأداة، وذلك كي تضمن استخدام نسخة محددة من إطار العمل أو المكتبة التي تم إنشاء هذه الإضافة لها. لتجنب مشاكل التوافقيّة بينهما.

# كيف أبدأ؟

## مُتطلّبات الاستخدام
يحتاج Bower إلى وجود بيئة **nodejs** مع مدير الحزم الخاصّ بها **npm** وإلى نظام  **git** على الحاسوب المراد تنصيب الأداة عليه.
...
إن كنت تعمل على Linux يمكنك اتباع الطريقة السابقة. أو يمكن تنصيب nodejs عن طريق المستودعات الخاصّة بنظام التشغيل، مثلاً إن كنت تستخدم Ubuntu أو أحد مشتقاته، يمكن تنصيب nodejs عن طريق سطر الأوامر:
```bash
...
إن كنت تعمل على Linux يمكن تنصيب git باستخدام المستودعات الخاصّة بنظام التشغيل، في حال استخدام توزيعة Ubuntu أو أحد مشتقاتها، يمكن تنصيب git عن طريق الأمر:
```bash
...
إن كنا نريد تنصيب مكوّن شهير، فغالباً ما تكون أول نتيجة بحث هي الحزمة المطلوبة، أول نتيجة تظهر في البحث السابق هي:
```
...
بعد أن أصبح تنصيب الحزم واضحاً، ماذا لو قمتَ بتنصيب حزمة ثمّ لم تعد تريدها، أو قمتَ بتنصيب حزمة خاطِئة غير التي تريدها؟
...
لنفرض أنّك تعمل ضمن فريق، وقمتَ بتنصيب آخر نسخة من jQuery وآخر نسخة من Twitter Bootstrap، ثمّ بعد شهرين قام زميلك في العمل بتنصيب نفس الحزم. لكنّه تفاجئ أن هناك أخطاء بدأت بالظهور لم تكن تظهر من قبل. بدأ يبحث عن المشكلة وعن الحزم المنصبة، وجد أن إصدارات الحزم التي قام بتنصيبها تختلف عن إصدارات الحزم التي قمتَ بتنصيبها منذ شهرين، عندما كان المشروع يعمل بشكل جيد!

مثال آخر، لنفرض أنك تعمل في فريق، وأحد زملائك لا يعرف ما هي المُتطلّبات التي يحتاجها المشروع، هو يريد فقط أن يقوم بتجربة المشروع. سيحتاج إلى طريقة لتنصيب الحزم المطلوبة جميعها قبل أن يقوم بتشغيل المشروع ليرى كيف يبدو. لكنه غير مختص ولا يعرف كيف!

الحل للمثالين هو استخدام ملف `bower.json`.

ما هو هذا الملف؟
هو ملف يقوم بتعريف الحزم المستخدم في المشروع مع رقم الإصدار الخاصّ بكل حزمة، ثمّ يمكن تنصيب الحزم عن طريق الأمر:
```
bower install
```
...
بهذه الطريقة يمكن لأي شخص من فريق العمل أن يقوم بتنصيب جميع المُتطلّبات بشكل بسيط وسهل دون أن يعرفوا الحزم وإصداراتها المطلوبة.
...
```
المقصود من هذا السؤال هو تحديد آلية عمل الوحدات المكوِّنة للمشروع (modules) عند كونه مشروع JavaScript متقدّماً.
...
```
هل تريد جعل الحزمة خاصّة بحيث لا يتم إضافة على مستودعات Bower؟ الإجابة الافتراضيّة هي نعم. إن أردت أن تكون الحزمة خاصّة دون نشرها يمكن كتابة كلمة `no` ثمّ ضغط زرّ `Enter`.
...
بهذا عند تنصيب أو تحديث الحزم، سيتم وضعها في مجلد `my_components` بدلاً من المجلد الافتراضيّّ.
...
- [WebStorm](https://www.jetbrains.com/webstorm)  بيئة متكاملة لتطوير مواقع الويب يتميز بتعامل قويّ جداً مع JavaScript، يملك هذا البرنامج دعماً مدمجاً به لأداة Bower.
...
- [Package Intellisense for Visual Studio 2013](https://visualstudiogallery.msdn.microsoft.com/65748cdb-4087-497e-a394-2e3449c8e61e)  إضافة مخصّصة لـ Visual Studio لإضافة دعم مدير حزم nodejs مع أداة Bower.

هناك أيضاً عدد من الإضافات الخاصّة بمشغّلات المهام `Grunt` و `Gulp` و `broccoli`.


## أي نوع من المشاريع يمكنني استخدام Bower معه؟

يمكنك استخدام Bower في أي مشروع ويب يحتاج إلى واجهاتٍ تظهر للمستخدم (Front-end). سواء كانت المكوِّنات للاستخدام المباشر أو لتتم معالجتها واستخدامها في بناء مكوّنات مخصّصة بناءً على مُتطلّبات التصميم.
