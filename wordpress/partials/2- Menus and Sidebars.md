...
قوائم التنقّل (Navigation Menu) هي إحدى ميزات القوالب (Themes)، توفّر ووردبريس طريقة سهلة للتحكم بالقوائم المخصصة للقالب من خلال لوحة التحكم (Dashboard)، نحتاج لإضافة بضعة أسطر برمجية فقط حتى نضيف دعم القوائم في القالب.
...
تقبل دالّة [`register_nav_menu`](http://codex.wordpress.org/Function_Reference/register_nav_menu) محدِّدَين هما: المكان (Location) و الوصف (Description).

- محدّد المكان (Location): يستخدم كمعرّف للقائمة، حيث يتم طلب محتوى القائمة ضمن ملفات القالب عن طريق محدّد المكان (location) الذي قمنا بتعيينه أثناء تسجيل القائمة. في حالتنا قمنا بوضع قيمة المحدد: **our-custom-menu**.
- محدد الوصف (Description): يتم استخدامه عند عرض القائمة في لوحة التحكم ليكون أنسب وأسهل للقراءة من محدد المكان، في حالتنا قمنا بوضع قيمة الوصف: **القائمة الرئيسية**.

يمكننا تسجيل أكثر من قائمة معاً عن طريق استخدام دالّة [`register_nav_menus`](http://codex.wordpress.org/Function_Reference/register_nav_menus) التي تُستَخدم بشكل مشابه لدالّة `register_nav_menu` لكنَّ المحدّدات تكون على شكل مصفوفة إسميّة (Associative Array)، كل عنصر في المصفوفة يمثّل قائمة واحدة بحيث يكون مفتاح العنصر هو محدّد المكان وقيمة العنصر هي محدّد الوصف.

فيما يلي مثال عن كيفية تسجيل أكثر من قائمة معاً:
```php
add_action('init', function() {
	register_nav_menus([
...
كما ننوه إلى وجود دالّة معاكسة لدالّة تسجيل القوائم هي [`unregister_nav_menu`](http://codex.wordpress.org/Function_Reference/unregister_nav_menu) التي يتم استخدامها لإلغاء تسجيل القوائم، فمثلاً في قالبٍ **ابن** (Child Theme) يمكن أن يتم إلغاء تسجيل قائمة تم تسجيلها في القالب **الأب** (Parent Theme)، أو إلغاء تسجيل قائمة من خلال إضافة (Plugin).

## إظهار القائمة في القالب
الخطوة الأخيرة في إضافة القوائم هي عرضها في المكان المناسب ضمن القالب.
في القالب الذي نستعمله نجد أن موقع القوائم أصبح في ملف `header.php`؛ في السطر 30 من الملف نجد وسم `<section class="top-bar-section">`، بداخله وسم `<ul>` نريد أن نضع القائمة بدلاً من وسم `<ul>`.
...
تقبل دالّة [`wp_nav_menu`](http://codex.wordpress.org/Function_Reference/wp_nav_menu) محدداً واحداً هو مصفوفة تحوي عدداً من الإعدادات، الإعداد الوحيد الضروري هو `theme_location` ويتم استخدامه كما في السطر البرمجي السابق. يمثّل هذا المحدّد قيمة محدّد المكان (Location) الذي استخدمناه أثناء تسجيل قائمة جديدة، كانت قيمة المكان: `our-custom-menu`.
في حال لم يتم إدخال هذا المحدّد تقوم ووردبريس باستخدام قيمة إعداد `menu` (سنأتي إليه في الفقرة التالية)، وإن لم تجد ووردبريس قيمة فسيتم عرض أول قائمة غير فارغة تجدها ووردبريس.
في حال عدم وجود أي قوائم *غير فارغة* وعدم تمرير محدّد المكان فلا يتم عرض شيء.
...
يمكن لدالّة `wp_nav_menu` أن تقبل محدّداً واحداً هو مصفوفة تحوي مجموعة إعدادات، لا يضطر المبرمج لإدخال مصفوفة الإعدادات بشكل كامل، يمكنه إدخال ما يحتاجه والباقي ستقوم ووردبريس بمعالجته وإسناد قيمته الافتراضية له. الإعدادات بشكل كامل هي:
...
بعد أن تعرّفنا على إعدادات إظهار القائمة، لنقم بتعديلها في قالبنا لتصبح مناسبة أكثر.
...
قمنا بجعل قيمة الوسم المحيط (container) تساوي `false` ﻷننا لا نريد إحاطة القائمة بأي وسم، فقالب HTML بشكل ابتدائي يقوم بإحاطة القائمة بوسم `<section>`.
وقمنا أيضاً بإضافة قيمة محدد `menu_class` لتساوي: `right`، ليتم إضافة صنف `right` لوسم القائمة `<ul>` كي يظهر بشكل جيّد.
...
لنقم معاً بإضافة الأسطر البرمجية إلى ملف `functions.php` لتعريف شريط جانبيّ جديد:
```php
...
**ملاحظة:** من المهم وضع معرّف للشريط الجانبي والقوائم بشكل دائم، ولو لم يستخدم القالب سوى شريط جانبي واحد أو قائمة واحدة.

## تخصيص الشريط الجانبي

كما لاحظنا فشكل ودجات (widgets) الشريط الجانبي غير مقبول، لنقم معاً بتحسين مظهرها عن طريق إحاطة كل واحدة من الودجات (widgets) بوسم `<div class="card">` الذي كان موجوداً في ملف `sidebar.php` قبل استخدام دالّة عرض الشريط الجانبي.
...
لم نقم في هذا الدرس بإضافة الكثير من الأمثلة حول إعدادات الشريط الجانبي أو القائمة بسبب عدم احتياج شكل القالب إلى المزيد من الإعدادات، لكننا أضفنا شرحاً عن كل واحد من الإعدادات ليكون مرجعاً للمطوّر إن كان يعمل على قالب بحاجة إلى المزيد من التخصيص.

بهذا نكون قد انتهينا من إضافة قائمة رئيسية وشريط جانبي إلى القالب، وشاهدنا كيف يمكن تخصيص شكل القائمة والشريط الجانبي ليتناسب مع القالب بأفضل شكل ممكن.